...
jvmtiError Redefiner::RedefineClassesDirect(ArtJvmTiEnv* env,
  ...
  // Don't accept any new JIT tasks. If the redefinition is structural then we have to discard
  // all the JITed code anyway. For in place redefinition we can allow JIT tasks if they don't
  // involve the method that is being redefined. Though we just stop accepting new requests in
  // both cases to keep the implementation simple.
  art::jit::ScopedJitPauseNewTasks jit_pause;
  art::jit::Jit* jit = art::Runtime::Current()->GetJit();
  if (jit != nullptr) {
    // Change the type of redefinition to structural if there's an ongoing compilation. We don't
    // have an easy way to cancel the ongoing JIT task and it is not safe to wait for the JIT task
    // to finish since it might be blocked on this thread. For in-place updates it is not safe to
    // change the method's code when a JIT compilation is in progress. Structural redefinition
    // allocates new structures, so it should be safe to let the JIT task finish and discard the
    // generated code.
    if (jit->GetThreadPool()->HasOngoingCompiles()) {
      type = RedefinitionType::kStructural;
    }
  }
...
jvmtiError Redefiner::Run() {
...

  {
    ...
    // Increment the redefinition count so code generated by any ongoing compilations would be
    // discarded. If an ongoing compilation at the start of redefinition has finished and installed
    // the code, the generated code would be discarded in UpdateClass along with all the JITed code.
    // If it finishes after this point, then the code is discarded when committing the code because
    // the redefinition count is incremented.
    runtime_->GetInstrumentation()->IncrementRedefinitionCount();
    for (RedefinitionDataIter data = holder.begin(); data != holder.end(); ++data) {
