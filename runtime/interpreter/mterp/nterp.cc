...
static ArtMethod* FindMethodFast(ArtMethod* caller,
  ...
  if (cls->GetDexTypeIndex() == method_id.class_idx_) {
    ArtMethod* method =
        cls->FindDeclaredClassMethod</* kOnlyLookAtIndex= */ false, kRuntimePointerSize>(
            method_index);
  ...
  ArtMethod* method = caller->GetDexCache()->GetResolvedMethod(method_index);
  if (CanAccessFastInternal(method, type, caller)) {
    return method;
  }
