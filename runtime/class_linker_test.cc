...
TEST_F(ClassLinkerTest, FindDeclaredClassMethodWithDuplicate) {
  ScopedObjectAccess soa(Thread::Current());
  StackHandleScope<1> hs(soa.Self());
  ObjPtr<mirror::Class> klass_obj = class_linker_->AllocClass(soa.Self(), sizeof(mirror::Class));
  Handle<mirror::Class> klass(hs.NewHandle(klass_obj));
  ASSERT_TRUE(klass != nullptr);
  ASSERT_EQ(0U, klass->NumMethods());

  // Test that we can get an empty array of methods.
  EXPECT_TRUE(klass->GetMethods(kRuntimePointerSize).empty());

  // Add 3 methods, with indices [0, 7, 7]
  constexpr size_t kNumMethods = 3;
  LengthPrefixedArray<ArtMethod>* methods = class_linker_->AllocArtMethodArray(
      soa.Self(), Runtime::Current()->GetLinearAlloc(), kNumMethods);
  ASSERT_TRUE(methods != nullptr);

  methods->At(0).SetDeclaringClass(klass_obj);
  methods->At(1).SetDeclaringClass(klass_obj);
  methods->At(2).SetDeclaringClass(klass_obj);

  methods->At(0).SetDexMethodIndex(0);
  methods->At(1).SetDexMethodIndex(7);
  methods->At(2).SetDexMethodIndex(7);
  methods->At(2).SetAccessFlags(kAccCopied);
  klass->SetMethodsPtr(methods, 3, 0);

  ASSERT_EQ(kNumMethods, klass->NumMethods());
  ArraySlice<ArtMethod> method_slice = klass->GetMethods(kRuntimePointerSize);
  ArtMethod* result =
      klass->FindDeclaredClassMethod</* kOnlyLookAtIndex= */ false, kRuntimePointerSize>(7);
  ASSERT_EQ(result, &methods->At(1));
  ASSERT_NE(result, &methods->At(2));
}

TEST_F(ClassLinkerTest, FindDeclaredClassMethodWithDuplicateOnlyFastPath) {
  ScopedObjectAccess soa(Thread::Current());
  StackHandleScope<1> hs(soa.Self());
  ObjPtr<mirror::Class> klass_obj = class_linker_->AllocClass(soa.Self(), sizeof(mirror::Class));
  Handle<mirror::Class> klass(hs.NewHandle(klass_obj));
  ...
  // Test that we can get an empty array of methods.
  EXPECT_TRUE(klass->GetMethods(kRuntimePointerSize).empty());
  // Add 4 methods, with indices [0, 1, 2, 1_copied]
  ...
  methods->At(1).SetDeclaringClass(klass_obj);
  methods->At(2).SetDeclaringClass(klass_obj);
  methods->At(3).SetDeclaringClass(klass_obj);
  methods->At(0).SetDexMethodIndex(0);
  methods->At(1).SetDexMethodIndex(1);
  methods->At(2).SetDexMethodIndex(2);
  methods->At(3).SetAccessFlags(kAccCopied);
  methods->At(3).SetDexMethodIndex(1);
  klass->SetMethodsPtr(methods, kNumMethods, 0);
  ASSERT_EQ(kNumMethods, klass->NumMethods());
  ArraySlice<ArtMethod> method_slice = klass->GetMethods(kRuntimePointerSize);
  ArtMethod* result =
      klass->FindDeclaredClassMethod</* kOnlyLookAtIndex= */ true, kRuntimePointerSize>(2);
  ASSERT_EQ(result, &methods->At(2));
  result = klass->FindDeclaredClassMethod</* kOnlyLookAtIndex= */ true, kRuntimePointerSize>(1);
  ASSERT_EQ(result, &methods->At(1));
}
